...
select *
from orders 
order by cast(concat(date,' ' ,time) as timestamp) desc;
```
...
order by cast(concat(date,' ' ,time) as timestamp) asc
;
```
...
where date = '2025-03-12'
	and time between '06:00:00' and '12:00:00'
;
```
...
```sql
select 
	od.pizza_id as "피자 ID",
	count(od.order_id) as "피자 주문 건 수"
...
select 
	od.pizza_id as "피자 ID",
	sum(od.quantity) as "총 주문 건 수량"
from order_details as od
group by od.pizza_id
having sum(od.quantity) >= 10
order by "총 주문 건 수량" desc
;
```
...
select
	o.date as date,
	count(od.pizza_id) as order_count,
...
```sql
-- https://troels.arvin.dk/db/rdbms/#select-top-n-standard
select 
	pr.pizza_id,
	p.type_id,
	p.size,
...
select
	date,
	count(o.date) as total_orders,
	sum(p.price * od.quantity) as total_orders
from orders as o
...
select 
	p.size,
	sum(p.price * od.quantity) as total_surplus
from order_details as od
	left join pizzas as p
		on p.id = od.pizza_id
group by p.size
